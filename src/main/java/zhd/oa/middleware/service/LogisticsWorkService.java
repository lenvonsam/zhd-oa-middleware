        ...
        try {
            ...
            if("".equals(msg)){
                for (int i = 0; i < detailJSONArray.size(); i++) {
                    ...
                    String closeCom = detailJSONObject.getString("closeCom");// 0:交投智联云  1:线下结算
                    String tranCom = detailJSONObject.getString("tranCom"); //运输单位名称
                    String driver = detailJSONObject.getString("driver"); //司机姓名
                    String phone = detailJSONObject.getString("phone");// 手机号
                    String carNo = detailJSONObject.getString("carNo");// 车牌号
                    String freightMode = detailJSONObject.getString("freightMode");// 0:不含税 1:含税
                    String price = detailJSONObject.getString("price");// 两位小数
                    String unit = detailJSONObject.getString("unit");// 0:/车 1:/吨
                    String weight = detailJSONObject.getString("weight");//三位小数
                    String money = detailJSONObject.getString("money");// 两位小数
                    String carMax = detailJSONObject.getString("carMax");//
                    String carLength = detailJSONObject.getString("carLength");//
                    String remk = detailJSONObject.getString("remk");//
                    ...
                    if(!"0".equals(closeCom) && !"1".equals(closeCom)){
                        msgDt += "结算单位不正确!";
                    }
                    ...
                    if(!phone.matches("^((13[0-9])|(14[5|7])|(15([0-3]|[5-9]))|(17[013678])|(18[0,5-9]))\\d{8}$")){
                        msgDt += "手机号不正确!";
                    }
                    if(!car){
                        msgDt += "车牌号不正确!";
                    }
                    if(!"0".equals(freightMode) && !"1".equals(freightMode)){
                        msgDt += "运费类型不正确!";
                    }
                    if(!price.matches("^(([1-9][0-9]{0,14})|([0]{1})|(([0]\\.\\d{1,2}|[1-9][0-9]{0,14}\\.\\d{1,2})))$")){
                        msgDt += "单价格式不正确!";
                    }
                    if(!"0".equals(unit) && !"1".equals(unit)){
                        msgDt += "运费单位不正确!";
                    }
                    if(!weight.matches("^[0-9]+\\.{0,1}[0-9]{0,3}$")){
                        msgDt += "吨位格式不正确!";
                    }
                    if(!money.matches("^(([1-9][0-9]{0,14})|([0]{1})|(([0]\\.\\d{1,2}|[1-9][0-9]{0,14}\\.\\d{1,2})))$")){
                        msgDt += "金额格式不正确!";
                    }
